...
# Detect custom SSH configuration
SSH_HOST="github.com"
if [ -f "$HOME/.ssh/github_infra_config" ]; then
    echo "🔍 Detected custom SSH config: ~/.ssh/github_infra_config"
    # Extract the Host alias from the config
    CUSTOM_HOST=$(grep "^Host " "$HOME/.ssh/github_infra_config" | head -1 | awk '{print $2}')
    if [ -n "$CUSTOM_HOST" ]; then
        echo "   Found SSH host alias: $CUSTOM_HOST"
        read -p "   Use this SSH configuration? (y/n): " use_custom
        if [[ "$use_custom" =~ ^[Yy]$ ]]; then
            SSH_HOST="$CUSTOM_HOST"
            echo "   ✅ Will use git@$SSH_HOST for remote URL"
        fi
    fi
    echo ""
elif [ -f "$HOME/.ssh/config" ]; then
    # Check for any github.com aliases in main SSH config
    GITHUB_ALIASES=$(grep "^Host.*github" "$HOME/.ssh/config" | grep -v "^Host github.com$" | awk '{print $2}' | head -5)
    if [ -n "$GITHUB_ALIASES" ]; then
        echo "🔍 Found GitHub SSH aliases in ~/.ssh/config:"
        echo "$GITHUB_ALIASES" | nl
        echo ""
        read -p "   Use a custom SSH host? (y/n, default: n): " use_alias
        if [[ "$use_alias" =~ ^[Yy]$ ]]; then
            echo "$GITHUB_ALIASES" | nl
            read -p "   Enter number (or type custom host): " alias_choice
            if [[ "$alias_choice" =~ ^[0-9]+$ ]]; then
                SSH_HOST=$(echo "$GITHUB_ALIASES" | sed -n "${alias_choice}p")
            else
                SSH_HOST="$alias_choice"
            fi
            echo "   ✅ Will use git@$SSH_HOST for remote URL"
        fi
        echo ""
...
if gh repo view "$REPO_FULL" &> /dev/null; then
    ...
    # Check if remote uses custom SSH host
    if git remote get-url origin &> /dev/null; then
        CURRENT_REMOTE=$(git remote get-url origin)
        if [ "$SSH_HOST" != "github.com" ] && [[ "$CURRENT_REMOTE" == "git@github.com:"* ]]; then
            echo "🔧 Updating remote URL to use custom SSH host..."
            NEW_REMOTE=$(echo "$CURRENT_REMOTE" | sed "s|git@github.com:|git@$SSH_HOST:|")
            git remote set-url origin "$NEW_REMOTE"
            echo "   ✅ Remote updated: $NEW_REMOTE"
        fi
    fi
else
    ...
    if [[ "$create_repo" =~ ^[Yy]$ ]]; then
        ...
        # Fix remote URL if using custom SSH host
